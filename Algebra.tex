...
\begin{multicols}{2}
	...
	\end{align*}
	 ...
	 \begin{itemize}
	 	...
	 	\begin{itemize}
	 		\item [$\{e\}  $.] Ovviamente abbiamo un omomorfismo banale.
	 		\item [$ \mathbb{Z}_2 $.] Il nucleo dev'essere un sottogruppo di indice 2 e il diedrale ne ha tre: $ \langle \rho \rangle, \langle \rho^2, \sigma \rangle, \langle \rho^2, \sigma\rho \rangle $.
...
Conosciamo già $ \Aut{D_4} $, cerchiamo, per concludere, di capire chi sia $ \Aut{Q_8} $. \\

Qui succederanno cose. \\
...
\[ \mathbb{Z}_2\times\mathbb{Z}_3\times\mathbb{Z}_5 \qquad D_{15} \qquad D_5\times\mathbb{Z}_3 \qquad D_{3}\times\mathbb{Z}_5 \]
...
Allora, se $ P_5 $ non è normale, ci sono sei 5-sylow, quindi 24 elementi di ordine 5. Tra i pochi elementi che rimangono non ci stanno sicuramente dieci 3-sylow e pertanto $ P_3 $ è normale. Allora $ P_3 $ e $ P_5 $ commutano, dunque
\[ P_3P_5 < G \]
...
\fun{\varphi}{\mathbb{Z}_2}{\mathbb{Z}_{15}}{y}{\varphi_y: x \mapsto yxy^{-1} = x^a}
sapendo che $ \phi_y^2(x) = x^{a^2} = x $, dobbiamo avere che $$  a^2 \equiv 1 \mod{15}  $$ e risolvendo il sistema di diofantee troviamo
\[ a = \pm 1,\, \pm 4 \mod{15} \]
e ognuna di questa azioni ci costruisce un prodotto diretto a uno dei gruppi trovati all'inizio.

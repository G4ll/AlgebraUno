...
\newcommand{\gen}[1]{\langle x \rangle}
...
\textsf{Questa parte dev'essere sistemata}

\iffalse Nel secondo caso dobbiamo avere $$  p = \text{ord}\left(\varphi_y\right) \mid \text{ord}(x^a) \mid q-1  $$
...
L'azione per coniugio è completamente determinata dal valore dell'automorfismo associato a un generatore $ y $ di $ K $, valutato su un generatore di $ x $ di $ H $
\[ \varphi_y: x \mapsto yxy^{-1} = x^a \]
che deve soddisfare $ a \in \Z_q $ e $ a^p \equiv 1 \mod{p} $, che sappiamo avere $ p-1 $ soluzioni.
...
\begin{multicols}{2}
	...
	\[ \begin{tikzcd}
	Q_8 \arrow[swap]{dr}{} & & & Q_8 \\
	D_4 \arrow[swap]{r}{} & G \arrow{r}{\varphi} & G \arrow[swap]{ur}{\Pi_Q}\arrow[swap]{r}{\Pi_D} & D_4
	\end{tikzcd}\]

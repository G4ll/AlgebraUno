...
\begin{multicols}{2}
	...
	\textsf{Questa pagina ha accumulato nel tempo una quantità tale di problemi che l'autore ha infine deciso di abbandonarla, questi si sono stratificati nel tempo e si presentano ora nella loro forma più temibile: l'omissione. Si consiglia cautela.}
	...
	\begin{proof}
		...
		ed essendo $ (\mu_\alpha) $ massimale e l'immagine non banale (perché stiamo quantomeno immergendo il campo), otteniamo $ (\mu_\alpha) = \ker{\Phi} $.
	\end{proof}
...
\end{theorem}
\begin{proof}
	 Il teorema \ref{est} ci dice che abbiamo tante estensioni quante le radici di $ \varphi\mu_\alpha $, ovvero $ n $.
\end{proof}
\begin{remark}
...
\begin{proof}
	...
	$ \Leftarrow $. Supponiamo che $ F $ sia il campo di spezzamento della famiglia di polinomi $ \{f_i\}_{i \in I} $, con radici $ \{\alpha_{i,j}\}_{i,j} $. Abbiamo che 
	\[F = K(\{\alpha_{i, j}\})\]
...
\end{proof}
 ...
 Chiediamoci ora come si comporta la normalità lungo le torri, prendiamone una
...
e chiediamoci:
\begin{itemize}
	\item Se L/K è normale, lo è anche L/F? 	Sì!
	
 Se $ \varphi $ è un $ F $-omomorfismo è anche un $ K $-omomorfismo, che già rispettava l'estensione $$  \varphi(L)=L  $$
...
\begin{multicols}{2}
	...
	le quali, per separabilità, sono tante quante il grado dell'estensione
	\[ |\text{Aut}_K(E)| = [E : K]. \]
	Questo viene chiamato \emph{gruppo di Galois} dell'estensione
...
\end{prop}
	...
	\fun{\texttt{res}}{\Gal{L/K}}{\Gal{L^H/K}}{\sigma}{\sigma_{|L^H}}
